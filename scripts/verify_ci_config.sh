...
check "fastlane fails on upload limit" "grep -q 'UI.user_error!' fastlane/Fastfile"
check "fastlane checks upload quota before build" "grep -q 'assert_testflight_upload_allowed' fastlane/Fastfile"
check "fastlane accepts target build number" "grep -q 'TARGET_BUILD_NUMBER' fastlane/Fastfile"
check "workflow has no force_upload bypass" "! grep -q 'force_upload' .github/workflows/testflight.yml"
